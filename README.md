...
- Giảng viên nhập tối đa 100 sinh viên từ file Excel `.xlsx` có cột `Họ và tên` và cột `Lớp` (không bắt buộc).
- Tên đăng nhập dùng lớp làm tiền tố; mật khẩu được tạo ngẫu nhiên cho từng sinh viên.
- Giữ nguyên file danh sách để xuất lại với cột tài khoản, hoặc với hai cột `Điểm` và `Lỗi trong quá trình làm bài`.
- Ghi rõ trường hợp sinh viên rời bài sớm hoặc thoát toàn màn hình.
...
2. Mở **SQL Editor**, chạy lần lượt:
   ...
   - `supabase/migrations/202609130003_add_deck_practice_mode.sql`
   - `supabase/migrations/202609140004_add_student_rosters.sql`
...
Sau khi đăng nhập, giảng viên mở tab **Tài khoản & điểm** và tải file Excel danh sách lên. Mật khẩu sinh viên không được lưu dạng đọc được trong bảng dữ liệu; vì vậy cần tải file cấp tài khoản ngay sau mỗi lần tạo. File gốc được lưu theo quyền của giảng viên để có thể xuất kết quả về sau.
